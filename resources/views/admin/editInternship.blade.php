                    ...
                    <form id="pictureForm" enctype="multipart/form-data">
                        ...
                        @if($image->src != null)
                            <img class="image" id="editable"
                                 ...
                                 src="{{url('public/dashboard/upload_files/internship/')}}/{{$image->src}}">
                        @endif
                        @if($image->src == null)
                        ...
                        <input type="hidden" name="sliderId" value="{{$image->id}}">
                    </form>
                </div>
                <div class="modal-footer" >
                    <button type="button" class="btn btn-dark col-md-5 col-md-offset-1" data-dismiss="modal">بستن</button>
                    <button type="button" content="{{$image->id}}" id="editSliderPicture" class="btn btn-warning col-md-5" >ویرایش</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
                    ...
                    <table style="direction:rtl;text-align: center" id="example"
                        ...
                        <form id="editForm">
                            ...
                            <tr class="unit">
                                ...
                                <td class="col-md-4 "><input  class="form-control" style="width: 100%;" id="title" name="title" value="{{$image->title}}"></td>
                                ...
                                @if($image->active == 1)
                                    <td><a id="active" content="{{$image->active}}" name="{{$image->id}}" type="button"  data-content="غیر فعال" class="btn btn-danger" >غیر فعال</a></td>
                                @endif
                                @if($image->active == 0)
                                    <td><a id="active" content="{{$image->active}}" name="{{$image->id}}" type="button"  data-content="فعال" class="btn btn-success"> فعال</a></td>
                                @endif
                ...
                swal({
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            ({
                                cache: false,
                                url: "{{url('admin/editInternshipTitle')}}",
        ...
        <script>
            $(document).on('click','#editSliderPicture',function(){
                ...
                ({
                    cache        : false,
                    url          : "{{url('admin/editInternshipPicture')}}",
                    type         : "post",
                    data         : formData,
                    dataType     : "JSON",
                    processData  : false,
        ...
        <script>
            $(document).on('click','#active',function () {
                ...
                //alert(title);
                swal({
                        title:   " آیا در نظر دارید وضعیت تصویر را " +"(( "+ title +" ))"+  "  نمایید؟ ",
                        text: "",
                        type: "info",
                        showCancelButton: true,
                        confirmButtonColor: "	#5cb85c",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            ({
                                cache: false,
                                url: "{{url('admin/enableOrDisableInternship')}}",
                                type: "post",
