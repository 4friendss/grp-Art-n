    ...
    <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                ...
                <a href="{{url('admin/addInternshipImage')}}" id="user-send" type="button" class="col-md-2 col-md-offset-5 btn btn-info" style=" font-weight: bold;">
                    <i class="fa fa-th-list"></i>                    افزودن تصویر جدید                </a>
                    ...
                    <table style="direction:rtl;text-align: center" id="example"
                        ...
                        @foreach($images as $image)
                            <tr class="unit">
                                ...
                                <td style="font-size: 120%">{{$image->title}}</td>
                                @if($image->src == null)
                                    <td style="font-size: 120%">تصویر ندارد</td>
                                @endif
                                @if($image->src != null)
                                    <td><button class="btn btn-basic" content="{{$image->src}}" id="showPicture">مشاهده تصویر</button></td>
                                @endif
                                @if($image->active == 1)
                                ...
                                @endif
                                <td><strong><a class="btn btn-warning col-md-8 col-md-offset-2"   href="{{url('admin/editInternship/'.$image->id)}}">ویرایش</a></strong> </td>
        ...
        <script>
            $(document).on('click','#showPicture',function(){
                ...
                $('#image').attr("src","{{url('public/dashboard/upload_files/internship/')}}"+'/'+src);
                $('#myModal').modal('show');
            })
        </script>
@endsection
